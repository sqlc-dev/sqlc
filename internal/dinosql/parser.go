...
const (
	MySQLCommentStyle CommentStyle = iota
	PGCommentStyle
)

func ParseMetadata(t string, commentStyle CommentStyle) (string, string, error) {
	for _, line := range strings.Split(t, "\n") {
		if commentStyle == PGCommentStyle && !strings.HasPrefix(line, "-- name:") {
			continue
		}
		if commentStyle == MySQLCommentStyle && !strings.HasPrefix(line, "/* name:") {
			continue
		}
		part := strings.Split(strings.TrimSpace(line), " ")

		if commentStyle == MySQLCommentStyle {
...
func parseQuery(c core.Catalog, stmt nodes.Node, source string) (*Query, error) {
	...
	name, cmd, err := ParseMetadata(strings.TrimSpace(rawSQL), PGCommentStyle)
	if err != nil {
		return nil, err
	}
