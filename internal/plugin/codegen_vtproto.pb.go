...
func (m *CodeGenRequest) CloneVT() *CodeGenRequest {
	...
	if rhs := m.GlobalOptions; rhs != nil {
		tmpBytes := make([]byte, len(rhs))
		copy(tmpBytes, rhs)
		r.GlobalOptions = tmpBytes
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (this *CodeGenRequest) EqualVT(that *CodeGenRequest) bool {
	...
	if string(this.GlobalOptions) != string(that.GlobalOptions) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *CodeGenRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.GlobalOptions) > 0 {
		i -= len(m.GlobalOptions)
		copy(dAtA[i:], m.GlobalOptions)
		i = encodeVarint(dAtA, i, uint64(len(m.GlobalOptions)))
		i--
		dAtA[i] = 0x32
	}
...
func (m *CodeGenRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field GlobalOptions", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.GlobalOptions = append(m.GlobalOptions[:0], dAtA[iNdEx:postIndex]...)
			if m.GlobalOptions == nil {
				m.GlobalOptions = []byte{}
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
