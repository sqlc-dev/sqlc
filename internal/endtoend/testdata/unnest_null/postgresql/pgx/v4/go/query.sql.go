...
func (q *Queries) CreateMemories(ctx context.Context, vamprieID []uuid.NullUUID) ([]Memory, error) {
	rows, err := q.db.Query(ctx, createMemories, vamprieID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetVampireIDs(ctx context.Context, vampireID []uuid.NullUUID) ([]uuid.UUID, error) {
