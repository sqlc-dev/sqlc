...
const findShoeSizes = `-- name: FindShoeSizes :many
SELECT id, first_name, last_name, age, shoe_size, shirt_size FROM users
WHERE shoe_size = ANY($1::size[])
`

func (q *Queries) FindShoeSizes(ctx context.Context, db DBTX, shoeSize []Size) ([]User, error) {
	rows, err := db.Query(ctx, findShoeSizes, shoeSize)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
