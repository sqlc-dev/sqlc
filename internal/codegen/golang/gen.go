...
func generate(settings config.CombinedSettings, enums []Enum, structs []Struct, queries []Query) (map[string]string, error) {
	...
	tmpl := template.Must(template.New("table").Funcs(funcMap).Parse(func() string {
		if golang.EmitGroupByFile {
			if golang.EmitInterface {
				return groupInterfaceTemplateSet
			}
			return groupTemplateSet
		}
		return templateSet
	}()))
	...
	execute := func(sourceName, structName, templateName string) error {
		var b bytes.Buffer
		w := bufio.NewWriter(&b)
		tctx.SourceName = sourceName
		tctx.StructName = structName
		...
		output[sourceName] = string(code)
		return nil
	}

	if err := execute("db.go", "Undeclared", "dbFile"); err != nil {
		return nil, err
	}
	if err := execute("models.go", "Undeclared", "modelsFile"); err != nil {
		return nil, err
	}
	if !golang.EmitGroupByFile && golang.EmitInterface {
		if err := execute("querier.go", "Undeclared", "interfaceFile"); err != nil {
			return nil, err
		}
	}
	...
	for _, gq := range queries {
		files[gq.SourceName] = gq.StructName
	}

	for sourceName, structName := range files {
		if err := execute(sourceName, structName, "queryFile"); err != nil {
			return nil, err
		}
	}
	return output, nil
