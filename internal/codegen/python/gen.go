...
func subscriptNode(value, slice string) *pyast.Node {
	return &pyast.Node{
		Node: &pyast.Node_Subscript{
			Subscript: &pyast.Subscript{
				Value: &pyast.Name{Id: value},
				Slice: &pyast.Name{Id: slice},
			},
		},
	}
}
...
func buildModelsTree(ctx *pyTmplCtx, i *importer) *pyast.Node {
	...
	for _, m := range ctx.Models {
		...
		for _, f := range m.Fields {
			var ann *pyast.Node
			if f.Type.IsArray {
				ann = subscriptNode("List", f.Type.InnerType)
			} else if f.Type.IsNull {
				ann = subscriptNode("Optional", f.Type.InnerType)
			} else {
