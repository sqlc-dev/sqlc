...
// DefaultVersion is the ClickHouse release the dialect is generated from.
// Bumping it is a deliberate change: the query tree format and type
// inference can shift between releases, so regenerate and review the dialect
...
func Locate() (string, error) {
	...
	if _, err := os.Stat(path); err != nil {
		return "", fmt.Errorf("clickhouse %s is not installed: run `go run ./cmd/goldeneye install clickhouse` in internal/goldeneye, or set CLICKHOUSE to a clickhouse binary", DefaultVersion)
	}
	return path, nil
}
