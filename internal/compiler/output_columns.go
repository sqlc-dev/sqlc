...
func (c *Compiler) outputColumns(qc *QueryCatalog, node ast.Node) ([]*Column, error) {
	...
	for _, target := range targets.Items {
		...
		resCols, err := c.outputColumn(qc, tables, res)
		if err != nil {
			return nil, err
		}
		cols = append(cols, resCols...)
	}
...
// outputColumn resolves the output column(s) produced by a single SELECT (or
// RETURNING/etc.) target. Most targets produce one column; `table.*` produces
// several. Also reused to type each sqlc.jsonb_build_object(...) value.
func (c *Compiler) outputColumn(qc *QueryCatalog, tables []*Table, res *ast.ResTarget) ([]*Column, error) {
	var cols []*Column
