...
import (
	...
	"github.com/kyleconroy/sqlc/examples/group/postgresql/pggroupstmt"
...
func TestBooks(t *testing.T) {
	...
	// create an author
	a, err := func() (retModel pggroupstmt.Author, retErr error) {
		dbauthors := pggroupstmt.NewAuthors(db)
	...
	// create transaction
	b3, err := func() (retModel pggroupstmt.Book, retErr error) {
		tx, err := db.Begin()
		if err != nil {
			retErr = err
			return
		...
		txbooks := pggroupstmt.NewBooks(tx)
		...
		// save first book
		now := time.Now()
		_, err = txcreate.Exec(ctx, pggroupstmt.BooksCreateParams{
			AuthorID:  a.AuthorID,
			Isbn:      "1",
			Title:     "my book title",
			BookType:  pggroupstmt.BookTypeFICTION,
			Year:      2016,
			Available: now,
			Tags:      []string{},
		})
		...
		// save second book
		b1, err := txcreate.Exec(ctx, pggroupstmt.BooksCreateParams{
			AuthorID:  a.AuthorID,
			Isbn:      "2",
		...
		err = txupdate.Exec(ctx, pggroupstmt.BooksUpdateParams{
			BookID: b1.BookID,
		...
		// save fourth book
		b3, err := txcreate.Exec(ctx, pggroupstmt.BooksCreateParams{
			AuthorID:  a.AuthorID,
			Isbn:      "4",
			Title:     "4th place finisher",
			BookType:  pggroupstmt.BookTypeNONFICTION,
			Year:      2011,
			Available: now,
			Tags:      []string{"other"},
		})
	...
	err = func() (retErr error) {
		dbbooks := pggroupstmt.NewBooks(db)
		...
		retErr = dbupdate.Exec(ctx, pggroupstmt.BooksUpdateISBNParams{
			BookID: b3.BookID,
			Isbn:   "NEW ISBN",
	...
	err = func() (retErr error) {
		...
		books0, err := dblist.Exec(ctx, pggroupstmt.BooksListByTitleYearParams{
			Title: "my book title",
			Year:  2016,
		})
		if err != nil {
