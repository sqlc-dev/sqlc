...
import (
	...
	"github.com/kyleconroy/sqlc/examples/group/postgresql/pggroupiface"
...
func TestBooks(t *testing.T) {
	...
	dbauthors := pggroupiface.NewAuthors(db)
	...
	txbooks := pggroupiface.NewBooks(tx)

	// save first book
	now := time.Now()
	_, err = txbooks.Create(ctx, pggroupiface.BooksCreateParams{
		AuthorID:  a.AuthorID,
		Isbn:      "1",
		Title:     "my book title",
		BookType:  pggroupiface.BookTypeFICTION,
		Year:      2016,
		Available: now,
		Tags:      []string{},
	})
	...
	// save second book
	b1, err := txbooks.Create(ctx, pggroupiface.BooksCreateParams{
		AuthorID:  a.AuthorID,
		Isbn:      "2",
	...
	err = txbooks.Update(ctx, pggroupiface.BooksUpdateParams{
		BookID: b1.BookID,
	...
	// save fourth book
	b3, err := txbooks.Create(ctx, pggroupiface.BooksCreateParams{
		AuthorID:  a.AuthorID,
		Isbn:      "4",
		Title:     "4th place finisher",
		BookType:  pggroupiface.BookTypeNONFICTION,
		Year:      2011,
		Available: now,
		Tags:      []string{"other"},
	})
	...
	dbbooks := pggroupiface.NewBooks(db)

	// upsert, changing ISBN and title
	err = dbbooks.UpdateISBN(ctx, pggroupiface.BooksUpdateISBNParams{
		BookID: b3.BookID,
		Isbn:   "NEW ISBN",
	...
	// retrieve first book
	books0, err := dbbooks.ListByTitleYear(ctx, pggroupiface.BooksListByTitleYearParams{
		Title: "my book title",
		Year:  2016,
	})
	if err != nil {
