...
import (
	...
	"github.com/kyleconroy/sqlc/examples/group/mysql/mygroupstmt"
...
func TestBooks(t *testing.T) {
	...
	authorID, err := func() (retID int32, retErr error) {
		dbauthors := mygroupstmt.NewAuthors(db)
	...
	bookThreeID, err := func() (retID int32, retErr error) {
		...
		txbooks := mygroupstmt.NewBooks(tx)
		...
		// save first book
		now := time.Now()
		_, err = txcreate.Exec(ctx, mygroupstmt.BooksCreateParams{
			AuthorID:  authorID,
			Isbn:      "1",
			Title:     "my book title",
			BookType:  mygroupstmt.BooksBookTypeFICTION,
			Yr:        2016,
			Available: now,
		})
		if err != nil {
		...
		// save second book
		result, err := txcreate.Exec(ctx, mygroupstmt.BooksCreateParams{
			AuthorID:  authorID,
			Isbn:      "2",
		...
		err = txupdate.Exec(ctx, mygroupstmt.BooksUpdateParams{
		...
		result, err = txcreate.Exec(ctx, mygroupstmt.BooksCreateParams{
			...
			BookType:  mygroupstmt.BooksBookTypeNONFICTION,
			Yr:        2011,
			Available: now,
			Tags:      "other",
		})
	...
	err = func() (retErr error) {
		dbbooks := mygroupstmt.NewBooks(db)
		...
		retErr = dbupdate.Exec(ctx, mygroupstmt.BooksUpdateISBNParams{
			BookID: bookThreeID,
			Isbn:   "NEW ISBN",
	...
	err = func() (retErr error) {
		...
		books0, err := dblist.Exec(ctx, mygroupstmt.BooksListByTitleYearParams{
			Title: "my book title",
			Yr:    2016,
		})
		if err != nil {
