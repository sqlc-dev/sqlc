...
import (
	...
	"github.com/kyleconroy/sqlc/examples/group/mysql/mygroup"
...
func TestBooks(t *testing.T) {
	...
	dbauthors := mygroup.NewAuthors(db)
	...
	txbooks := mygroup.NewBooks(tx)

	// save first book
	now := time.Now()
	_, err = txbooks.Create(ctx, mygroup.BooksCreateParams{
		AuthorID:  int32(authorID),
		Isbn:      "1",
		Title:     "my book title",
		BookType:  mygroup.BooksBookTypeFICTION,
		Yr:        2016,
		Available: now,
	})
	if err != nil {
	...
	// save second book
	result, err = txbooks.Create(ctx, mygroup.BooksCreateParams{
		AuthorID:  int32(authorID),
		Isbn:      "2",
	...
	err = txbooks.Update(ctx, mygroup.BooksUpdateParams{
	...
	result, err = txbooks.Create(ctx, mygroup.BooksCreateParams{
		...
		BookType:  mygroup.BooksBookTypeNONFICTION,
		Yr:        2011,
		Available: now,
		Tags:      "other",
	})
	...
	dbbooks := mygroup.NewBooks(db)

	// upsert, changing ISBN and title
	err = dbbooks.UpdateISBN(ctx, mygroup.BooksUpdateISBNParams{
	...
	// retrieve first book
	books0, err := dbbooks.ListByTitleYear(ctx, mygroup.BooksListByTitleYearParams{
		Title: "my book title",
		Yr:    2016,
	})
	if err != nil {
